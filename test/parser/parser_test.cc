...
TEST(ParserTest, BasicProgram) {
    ...
    std::shared_ptr<FunctionNode> main_function = program->functions_[0];
    ...
    auto return_stmt = std::dynamic_pointer_cast<ReturnStatementNode>(main_function->body_->statements_[0]);
    ASSERT_NE(return_stmt, nullptr);
    auto return_value = std::dynamic_pointer_cast<IntegerValueNode>(return_stmt->return_value_);
